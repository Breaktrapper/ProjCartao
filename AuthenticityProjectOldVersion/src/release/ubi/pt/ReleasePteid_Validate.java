...
public class ReleasePteid_Validate {
    ...
    public ReleasePteid_Validate(boolean debug) {
    ...
    public void refresh() throws CardException, IOException, PteidException {
        ...
        if (certificates.length != 1 || digestAlgorithmIds.length != 1 || !digestAlgorithmIds[0].getName().equals("SHA256")) { --------> Não funciona porque existe mais que um certificado no cartão..
        ...
        this.fourHashDataDigest = sha256Instance.digest(dataBytes);
        if (debugMode) {
        ...
        //ghet the signature
        this.signature = signerInfos[0].getEncryptedDigest();
        ...
        try {
            ...
            if (debugMode) {
                System.out.println("decipher :" + ReleaseUtils.bytesToHex(cipherData));
            }
        ...
        if (debugMode) {
            System.out.println("signedDataDigest" + ReleaseUtils.bytesToHex(signedDataDigest));
        }
